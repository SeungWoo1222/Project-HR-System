    ...
    SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        http
                ...
                .authorizeHttpRequests(authorizeRequests ->
                        authorizeRequests
                                .requestMatchers("/actuator/health", "/dev/**", "/auth/login", "/auth/logout", "/auth/join","/auth/session-expired", "/error/**","/css/**", "/js/**", "/images/**", "/files/**", "/api/employee/update", "/api/employee/join").permitAll() // 이 경로는 인증 없이 접근 허용
...
//                              .anyRequest().permitAll() // 모든 요청에 대해 인증 없이 접근 허용
                ...
                // 새로운 방식의 HTTP Basic 인증 활성화 - PostMan 로그인..
                .httpBasic(httpBasic ->
                        httpBasic
